...
So `<Placeholder />` is loud, striped like roadwork — this section of road is
not finished. `<Verify />` marks a sourced-but-unconfirmed fact with a dotted
underline, and `<VerifyNote />` carries the reason beside it.
...
- **No invented facts.** Legal name, EIN, addresses, dates and bios are holes.
